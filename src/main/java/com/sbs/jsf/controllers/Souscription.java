...
public class Souscription implements Serializable {
    ...
    public void init() {
        ...
        try {
            ...
            List<Operateurs> lop = operateursFacade.findByDesignationOperateur("ORANGE");
            if (!lop.isEmpty()) {
                orange = lop.get(0);
            } else {
                throw new OperatorNotFoundException("ORANGE");
            }
    ...
    public void checkKYC() {
        ...
        try {
            ...
            regpt.KYCRequest(msisdn, activationKey, status, lastname, firstname, dob, cin);
            ...
            abOm.setPrenoms(firstname.value);
            abOm.setNom(lastname.value);
    ...
    public void checkKycFromBankSideTest() {
...
//
//                }
//            } else { //si c un abonnement wizall qui est deja la 
//
//                System.out.println("on peux souscrire le client avec ce numero de compte et ce numero de telephone car c un abonnement wizall seulement qui est la ");
//                 callWebServiceAndRegisterTest();
//                    newAbonnement = new Abonnements();
//                    abOm = new AbonnementsOm();
//                    listAbBque.clear();
//                    libelleCodeRetour = "";
//            }
            //si operateur different de orange : on affiche le msg d'info et on fait la souscription 
//            if (!listOp.contains("ORANGE")) {
//
//                System.out.println("attention abonnement en cours avec un operateur wizall");
//                //callWebServiceAndRegister();
//                callWebServiceAndRegisterTest();
//                newAbonnement = new Abonnements();
//                abOm = new AbonnementsOm();
...
//                libelleCodeRetour = "";
//
//            } else {//si il y a deja un abonnement en cours avec l'opérateur orange , on permet cet nouvel abonnement que si il s'agit d'un autre numero de compte 
//                //on verifie le verou
//                //String unique_acount_alias = parametresFacade.findByACodeParam("UNIQUE_ACCOUNT_ALIAS");
//
//                //System.out.println("INTERDICTION DE LIER UN NUMERO MSISDN SUR PLUSIEURS COMPTE BANCAIRES  :  " + unique_acount_alias);
//                
//                  //si il est actif : alors on verifi si le numero de compte choisi est deja utilise : dans la liste des abonnments retournee 
//                    for (Abonnements ab : ListAbonnementActifs) {
//                        System.out.println("  " + ab.getNumerotelephone() + "  " + ab.getCompte() + "  operateur " + ab.getOperateur().getDesignationOperateur());
//                        numerosComptes.add(ab.getCompte());
//                    }
//
//                    if (numerosComptes.contains(newAbonnement.getCompte())) {//si le numero de compte choisi na jamais ete utilisee : msg derreur 
//                        addErrorMessage("ERREUR", "ABONNEMENT DEJA EXISTANT");
//                    } else {
//                        //addErrorMessage("ERREUR", "INTERDICTION DE LIER UN MEME NUMERO A PLUSIEUR COMPTE BANCAIRES ");
//                        addMessage("INFOS", "INSCRIPTION A FAIRE ....");
//                    }
//
//
//            }
...
//            System.out.println("erreur au niveau du test , revoir le code ");
    ...
    public void checkKYCFromBankSide() {
        try {
            ...
            try {
                //System.out.println("RECUPERATION DE CLIENT   " + client.getNom() + "-" + client.getPrenom());
                System.out.println("RECUPERATION DE CLIENT   " + client.getNom());
            ...
            //System.out.println("les infos du clients trouve " + newAbonnement.getNom() + " " + newAbonnement.getPrenoms() + " " + newAbonnement.getCni());
            System.out.println("les infos du clients trouve " + newAbonnement.getNom() + " " + newAbonnement.getCni());

            // selectedComptes=new Comptes();
            // System.out.println("CLIENT: " + client.getNom());
        } catch (ParameterNotFoundException | MalformedURLException e) {
        }
    }

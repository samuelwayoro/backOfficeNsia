 ...
 * @author samuel
 */
@ManagedBean(name = "parametrageMtnGbManager")
@ApplicationScoped
public class ParametrageMtnGbManager implements Serializable {
    ...
    public ParametrageMtnGbManager() {
    }
    ...
    public void init() {
        try {
            ...
            List<Parametres> l = parametresFacade.findByCodeParam("MODEIDLE_MTNGB");
            if (l != null && !l.isEmpty()) {
                modeIdle = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("MODEIDLE_MTNGB");
            }
            //list des pages pour les souscriptions
            listPagesouscription = pagesouscriptionFacade.findByOperateur("MTNGB");
            // Paramètre définissant l'activation ou non de la supervalidation
            l = parametresFacade.findByCodeParam("SUPERVALIDATION_MTNGB");
            if (l != null && !l.isEmpty()) {
                supervalidation = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("SUPERVALIDATION_MTNGB");
            }
            //supervalidation=parametrageManager.getSupervalidation();
            //Paramètre définissant le mode de souscription
            l = parametresFacade.findByCodeParam("MODESOUSCRIPTION_MTNGB");
            if (l != null && !l.isEmpty()) {
                selectedModeSouscription = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("MODESOUSCRIPTION_MTNGB");
            }
            //Paramètre définissant le nombre max de transactions
            l = parametresFacade.findByCodeParam("NOMBRE_MAX_TRANSACTION_MTNGB");
            if (l != null && !l.isEmpty()) {
                nombreMaxTrancations = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("NOMBRE_MAX_TRANSACTION_MTNGB");
            }

            //Paramètre définissant le montant max de transactions
            l = parametresFacade.findByCodeParam("MONTANT_MAX_TRANSACTION_MTNGB");
            if (l != null && !l.isEmpty()) {
                montantMaxTrancations = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("MONTANT_MAX_TRANSACTION_MTNGB");
            }

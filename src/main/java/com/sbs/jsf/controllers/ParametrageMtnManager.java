 ...
 * @author SOCITECH-
 */
@ManagedBean(name = "parametrageMtnManager")
@ApplicationScoped
public class ParametrageMtnManager implements Serializable {
    ...
    public void init() {
        try {
           ...
           // System.out.println("LISTE VALEUR: " + listValeursSouscription.size());
            ...
            List<Parametres> l = parametresFacade.findByCodeParam("MODEIDLE_MTN");
            if (l != null && !l.isEmpty()) {
                modeIdle = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("MODEIDLE_MTN");
            }
            //list des pages pour les souscriptions
            listPagesouscription = pagesouscriptionFacade.findByOperateur("MTN");
...
//            }
            ...
            l = parametresFacade.findByCodeParam("MODESOUSCRIPTION_MTN");
            if (l != null && !l.isEmpty()) {
                selectedModeSouscription = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("MODESOUSCRIPTION_MTN");
            }
            //Paramètre définissant le nombre max de transactions
            l = parametresFacade.findByCodeParam("NOMBRE_MAX_TRANSACTION_MTN");
            if (l != null && !l.isEmpty()) {
                nombreMaxTrancations = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("NOMBRE_MAX_TRANSACTION_MTN");
            }

            //Paramètre définissant le montant max de transactions
            l = parametresFacade.findByCodeParam("MONTANT_MAX_TRANSACTION_MTN");
            if (l != null && !l.isEmpty()) {
                montantMaxTrancations = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("MONTANT_MAX_TRANSACTION_MTN");
            }
            ...
            System.out.println("VALEUR IDLE AU DEMARRAGE DE L'APPLI : " + modeIdle.getCodeparam());
            idle = !modeIdle.getValeur().equals("IDLE_OUI");
            supvalidation = supervalidation.getValeur().equals("SUPERVALIDATION_OUI");

            String[] pattern = new String[]{"KEYSTORE_MTN", "TRUSTSTORE_MTN"};
            ...
            if (paraList != null && !paraList.isEmpty()) {
                for (Parametres para : paraList) {
                    switch (para.getCodeparam()) {
                        case "CHEMIN_FICHIER_KEYSTORE_MTN":
                            sSLConfig.setKeystore(para.getValeur());
                            break;
                        case "CHEMIN_FICHIER_TRUSTSTORE_MTN":

...
import com.ph.tlc.mobilebank.AccountInfoRequest;
import com.ph.tlc.mobilebank.BankRegistrationService;
import com.ph.tlc.mobilebank.BankRegistrationServiceSoap;
import com.ph.tlc.mobilebank.GetKYCRequestResponse;
import com.ph.tlc.mobilebank.LinkAccountRequest2;
...
import com.sbs.easymbank.utility.JsonToObject;
...
@ManagedBean
@ViewScoped
public class SouscriptionMoov implements Serializable {
    ...
    private String operator_code_moov;
    ...
    public void init() {
        ...
        try {
            ...
            if (soapWrapper.CertificatePath == null) {
                throw new ParameterNotFoundException("CHEMIN_FICHIER_TRUSTSTORE");
            }
            ...
            p = parametresFacade.findByCodeParam("OPERATOR_CODE_MOOV");
            // String pass;
            if (!p.isEmpty()) {
                operator_code_moov = p.get(0).getValeur();
            } else {
...
//            }
    ...
    public String getOperator_code_moov() {
        return operator_code_moov;
    }

    public void setOperator_code_moov(String operator_code_moov) {
        this.operator_code_moov = operator_code_moov;
    }
    ...
    public Operateurs getMoov() {
        return moov;
    }

    public void setMoov(Operateurs moov) {
        this.moov = moov;
    }
    ...
    public void checkKYC() {
        try {
            ...
            //  System.out.println("user: "+loginManager.getUtilisateur().getLogin());
            ...
            //  List<Parametres> operatorCode = parametresFacade.findByCodeParam("USERNAME_MOOV");
            String response = SoapWrapper.getMsisdnKYC(token, operator_code_moov, msisdn, activationKey);
            StandardResp standardResp = SoapWrapper.getResponseProprety(response, "getKYCResponse");
            abOm.setCodeRetour(new BigInteger(Integer.toString(standardResp.getStatusCode())));
            libelleCodeRetour = standardResp.getMessage();
            if (!standardResp.getData().isEmpty()) {
                JsonToObject.abonnementFromMoovResponse(standardResp.getData(), abOm);
            }

        } catch (Exception ex) {
            abOm.setCodeRetour(new BigInteger("604"));
            codeRetour = 604;
...
//INTEGRATION
        ...
        if ((ListAbonnementActifs.isEmpty()) && (ListAbonnementActifs.size() == 0)) {//si il y a pas d'abonnement avec le numero de telephone saisie 
            ...
            if (parametrageMoovManager.getSupervalidation().getValeur().equals("SUPERVALIDATION_NON") && newAbonnement.getActif()) {
                try {
                    PDF();
            ...
            List<Abonnements> abn = abonnementsFacade.findAbonnementByCompteAndMsisdn(newAbonnement.getCompte(), msisdn, moov);
...
//        }
    ...
    public void callWebServiceAndRegisterTest() {
        ...
        try {
            ...
            System.out.println("lancement de la souscription test de MOOV test ----- valeur de coderesponse  " + codeResponse);
...
//            System.out.println("erreur au niveau du test , revoir le code ");
    ...
    public void callWebServiceAndRegister() {
        calendar = Calendar.getInstance();
        if (parametrageMoovManager.getSupervalidation().getValeur().equals("SUPERVALIDATION_OUI")) {
            /**
             * ***A COMMENTER*****
             */
            ...
            try {
                ...
                // List<Parametres> operatorCode = parametresFacade.findByCodeParam("USERNAME_MOOV");
                // newAbonnement.setAlias(StringUtils.stripAccents(newAbonnement.getAlias()));
                //         newAbonnement.setAlias(Normalizer.normalize(newAbonnement.getAlias(), Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]", ""));
                String response = SoapWrapper.linkAccountRequest(token, operator_code_moov, msisdn, activationKey, loginManager.getUtilisateur().getLogin(), newAbonnement.getAlias(), newAbonnement.getLabel(), "ABONNEMENT");
                StandardResp standardResp = SoapWrapper.getResponseProprety(response, "linkAccountResponse");
                newAbonnement.setCoderetour(Integer.toString(standardResp.getStatusCode()));
                libelleCodeRetour = standardResp.getMessage();
                System.out.println("INPUT:{msisdn:" + msisdn + ",alias:" + newAbonnement.getAlias() + ",activationKey:" + newAbonnement.getActivation() + ",serviceCode:" + newAbonnement.getService() + "} RETOUR LINKACCOUNTREQUEST: " + standardResp.getStatusCode() + standardResp.getMessage());
                codeRetour = new Short(newAbonnement.getCoderetour());
        ...
        try {
            ...
            if (client != null) {
                ...
                for (int i = 0; i < tab.length; i++) {
                    listPhone.add(tab[i].replaceAll("\\s+", ""));
                }
...
//    }
    ...
    public void initJasper() throws JRException, FileNotFoundException {
        ...
        List<Parametres> list = parametresFacade.findByCodeParam("FICHE_SOUSCRIPTION_MOOV");
        String reportPath = "";
        if (!list.isEmpty()) {

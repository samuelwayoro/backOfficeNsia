 ...
 * @author SOCITECH-
 */
@ManagedBean(name = "parametrageMalitelManager")
@ApplicationScoped
public class ParametrageMalitelManager implements Serializable {
    ...
    public ParametrageMalitelManager() {
    }
    ...
    public void init() {
        try {
           ...
           // System.out.println("LISTE VALEUR: " + listValeursSouscription.size());
            ...
            List<Parametres> l = parametresFacade.findByCodeParam("MODEIDLE_MALITEL");
            if (l != null && !l.isEmpty()) {
                modeIdle = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("MODEIDLE_MALITEL");
            }
            //list des pages pour les souscriptions
            listPagesouscription = pagesouscriptionFacade.findByOperateur("MALITEL");
            //Paramètre définissant l'activation ou non de la supervalidation
            l = parametresFacade.findByCodeParam("SUPERVALIDATION_MALITEL");
            if (l != null && !l.isEmpty()) {
                supervalidation = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("SUPERVALIDATION_MALITEL");
            }
//supervalidation=parametrageManager.getSupervalidation();
            //Paramètre définissant le mode de souscription
            l = parametresFacade.findByCodeParam("MODESOUSCRIPTION_MALITEL");
            if (l != null && !l.isEmpty()) {
                selectedModeSouscription = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("MODESOUSCRIPTION_MALITEL");
            }

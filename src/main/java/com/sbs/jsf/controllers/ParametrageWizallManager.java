 ...
 * @author SOCITECH-
 */
@ManagedBean(name = "parametrageWizallManager")
@ApplicationScoped
public class ParametrageWizallManager implements Serializable {
    ...
    public ParametrageWizallManager() {
    }
    ...
    public void init() {
        try {
            ...
            List<Parametres> l = parametresFacade.findByCodeParam("MODEIDLE_WIZALL");
            if (l != null && !l.isEmpty()) {
                modeIdle = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("MODEIDLE_WIZALL");
            }
            //list des pages pour les souscriptions
            listPagesouscription = pagesouscriptionFacade.findByOperateur("WIZALL");
            // Paramètre définissant l'activation ou non de la supervalidation
            l = parametresFacade.findByCodeParam("SUPERVALIDATION_WIZALL");
            if (l != null && !l.isEmpty()) {
                supervalidation = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("SUPERVALIDATION_WIZALL");
            }
            //supervalidation=parametrageManager.getSupervalidation();
            //Paramètre définissant le mode de souscription
            l = parametresFacade.findByCodeParam("MODESOUSCRIPTION_WIZALL");
            if (l != null && !l.isEmpty()) {
                selectedModeSouscription = l.get(0);
                l.clear();
            } else {
                throw new ParameterNotFoundException("MODESOUSCRIPTION_WIZALL");
            }
